...
namespace turkerHR
{
    public partial class frmKurslarSertifikalarEkle : Form
    {
        ...
        private string _turkerHRKursSertifikaID; //forma parametre alisverisi icin
        public string turkerHRKursSertifikaID
        {
            get { return _turkerHRKursSertifikaID; }
            set { _turkerHRKursSertifikaID = value; }
        } //http://www.vcskicks.com/data-between-forms.php

        OleDbConnection con;
        OleDbCommand objKomut;

        public frmKurslarSertifikalarEkle()
        {
            InitializeComponent();
        }

        private void frmKurslarSertifikalarEkle_Load(object sender, EventArgs e)
        {
            if (turkerHRKursSertifikaID != "-1")
            {
                tusKilidi();
            }
            label1.Text = "Kullanıcı ID ve İsim Soyad: " + _turkerHRInsanID + " " + _turkerHRIsim + " " + _turkerHRSoyad;
            label2.Text = "Referans ID: " + turkerHRKursSertifikaID;
            ...
            string sorgu1 = @"SELECT * FROM tblKurslarSertifikalar WHERE id=" + turkerHRKursSertifikaID + @";";
            ...
            while (sonuc.Read())
            {
                textBox1.Text = sonuc["kurs"].ToString();
                textBox2.Text = sonuc["sertifika"].ToString();
           ...
           else
           {
             */
               if (turkerHRKursSertifikaID != "-1")
               {
                   ...
                   con.Open();
                   string sorgu1 = @"UPDATE tblKurslarSertifikalar SET";
                   //sorgu1 += " id=" + turkerHRKursSertifikaID + ",";

                   sorgu1 += " idinsanlar=" + turkerHRInsanID + ",";
                   sorgu1 += " kurs='" + textBox1.Text + "',";
                   sorgu1 += " sertifika='" + textBox2.Text + "',";
                   sorgu1 += " baslangictarihi='" + textBox3.Text + "',";
                   sorgu1 += " bitistarihi='" + textBox4.Text + "'";
                   sorgu1 += " WHERE id=" + turkerHRKursSertifikaID.ToString() + @";";
               ...
               else // yani yeni kayıt
               {
                   ...
                   string sorgu1 = @"INSERT INTO tblKurslarSertifikalar ";
                   sorgu1 += "(";
                   sorgu1 += "idinsanlar, kurs, sertifika, baslangictarihi, bitistarihi";
                   sorgu1 += ") VALUES (";
                   // sorgu1 += "VALUES (";
                   // sorgu1 += "NULL, ";
