...
namespace turkerHR
{
    public partial class frmPozisyonlarDiger : Form
    {
        ...
        public frmPozisyonlarDiger()
        {
            InitializeComponent();
        }

        private void frmPozisyonlarDiger_Load(object sender, EventArgs e)
        {
            label1.Text = turkerHRInsanID.ToString();
            label2.Text = turkerHRIsim.ToString() + " " + turkerHRSoyad.ToString();

            string sorgu1 = "SELECT count(*) FROM idinsanlar_idpozisyonlardiger WHERE idinsanlar=" + turkerHRInsanID.ToString() + ";";
            ...
            con.Open();
            string sql = "SELECT idpozisyonlardiger FROM idinsanlar_idpozisyonlardiger WHERE idinsanlar=" + turkerHRInsanID.ToString() + ";";
            ...
            for (int k = 0; k < bildigiToplamMeslekSayisi; k++)
            {
                ...
                bildigiMeslekIdleri[k] = Convert.ToInt32(sonuc["idpozisyonlardiger"]) - 1; // - 1 koydum çünkü koymazsam bir checkbox ileridekini seçiyor. düzeltmek maksadıyla.
            ...
            for (int i = 0; i < mesleksayisi; i++)
            {
                ...
                boxes[i].Text = sonuc2["meslek"].ToString();
        ...
        {
            con.Open();
            string sorgu3 = "SELECT count(*) FROM pozisyonlardiger;";
            objKomut.CommandText = sorgu3;
            int mesleksayisi = Convert.ToInt32(objKomut.ExecuteScalar());
                    ...
                    if (((CheckBox)kontrol).Name.ToString().Equals("kutu" + sayi.ToString())) // benim dinamik nesnelerimin isimleri kutu1,kutu2,... şeklinde.
                    {
                        if (((CheckBox)kontrol).Checked == true) // sadece seçili olan kutular.
                        {
                            ...
                            try
                            {
                                ...
                                string sorgu6 = "SELECT DISTINCT idinsanlar,idpozisyonlardiger FROM idinsanlar_idpozisyonlardiger WHERE ";
                                sorgu6 += "idinsanlar=" + turkerHRInsanID.ToString() + " AND ";
                                sorgu6 += "idpozisyonlardiger=" + (sayi + 1).ToString() + ";";
                                ...
                                {
                                    string sorgu4 = "INSERT INTO idinsanlar_idpozisyonlardiger(idinsanlar, idpozisyonlardiger) VALUES (" + turkerHRInsanID.ToString() + ", " + (sayi + 1).ToString() + ");";
                        ...
                        else if (((CheckBox)kontrol).Checked == false) // sadece seçili olan kutular.
                        {
                            ...
                            string sorgu4 = "DELETE FROM idinsanlar_idpozisyonlardiger WHERE idinsanlar=" + turkerHRInsanID.ToString() + " AND idpozisyonlardiger=" + (sayi + 1).ToString() + ";";
                            objKomut.CommandText = sorgu4;
                            int sonuc5 = Convert.ToInt32(objKomut.ExecuteScalar());
                        }
                    }
